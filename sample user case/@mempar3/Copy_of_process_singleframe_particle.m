...
%     ub=[min(hs+1+maxdisp,2*hs+1),min(hs+1+maxdisp,2*hs+1),min(cnt(inuc,3)-zcut+maxdisp/zxr,size(bimg3,3)),4,6,2];
    lb=[1,1,1,.1,.1];
    ub=[1+2*hs,1+2*hs,1+2*hs,3,2];
    options = optimset('Algorithm','trust-region-reflective','TolX',1e-3,...
    'MaxIter',1e4,'TolFun',1e-8,'Display','final');
    ...
    [param1,resnorm,~,exitflag1,~,~,~] = lsqnonlin(gaussn,ini,lb,ub,options,X,Y,Z,bimg3);
    ...
    [param2,fval,exitflag1] = fmincon(fmin,ini2,[],[],[],[],lb2,ub2);
...
%     resnorm2
%     resnorm1
    if resnorm2<resnorm1 && param2(1,5)>0.4 && param2(2,5)>0.4 && dist_2p>0.05 && dist_2p<5
        isdouble=1;
    else
        isdouble=0;
    end
