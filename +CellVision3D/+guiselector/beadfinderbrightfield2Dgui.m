function [out] = beadfinderbrightfield2Dgui( img,varargin )
%find the beads in bright field
...
out.cnt=hdata.data.cnt;
out.r=hdata.data.cnt;
...
choose = as>200 & as<10000 &...
    xs> border & ys>border & ...
    xs< size(img,2)-border & ys<size(img,1)-border & ps<6 &...
   sa>20 & sa<60 & es<.4 ;%& ps<3 ;%& es>.2; %sa < 60 & 
p=p(choose);
bb=[];
for i=1:length(p)
    bw4=zeros(size(bwn));
...
hdata.data.bb=bb;
hdata.data.cnt=[xs(choose)',ys(choose)'];
hdata.data.radius=sqrt(as(choose)/pi);
end
